classdef BeeParameters
    %BEEPARAMETERS Summary of this class goes here
    ...
    properties (Access=public)
        ...
        Algorithms;
        Debug;
        ProbabilityToChooseWorseNeighbor;
    end

    methods
        function self = BeeParameters(kvargs)
            %BEEPARAMETERS Construct an instance of this class
            %   Detailed explanation goes here
            arguments
                kvargs.NumScoutBees (1, 1) double {...
                    mustBePositive(kvargs.NumScoutBees), ...
                    mustBeInteger(kvargs.NumScoutBees)} = 14;
                kvargs.NumBestSolutions (1, 1) double {...
                    mustBePositive(kvargs.NumBestSolutions), ...
                    mustBeInteger(kvargs.NumBestSolutions)} = 14;
                kvargs.NumBestSolutionNeighbors (1, 1) double {...
                    mustBePositive(kvargs.NumBestSolutionNeighbors), ...
                    mustBeInteger(kvargs.NumBestSolutionNeighbors)} = 10;
                kvargs.NumEliteSolutions (1, 1) double {...
                    mustBePositive(kvargs.NumEliteSolutions), ...
                    mustBeInteger(kvargs.NumEliteSolutions)} = 4;
                kvargs.NumEliteSolutionNeighbors (1, 1) double {...
                    mustBePositive(kvargs.NumEliteSolutionNeighbors), ...
                    mustBeInteger(kvargs.NumEliteSolutionNeighbors)} = 20;
                kvargs.MaxIter (1, 1) double {...
                    mustBePositive(kvargs.MaxIter), ...
                    mustBeInteger(kvargs.MaxIter)} = 10000;
                % kvargs.Algorithms (1, :) double {mustBeNonnegative} = [6,7,8,9,10,11,14,17,18,19,20,21,23,28];
                kvargs.Algorithms (1, :) double {mustBeNonnegative} = 0;
                kvargs.ProbabilityToChooseWorseNeighbor (1, 1) double {mustBeNonnegative} = 0.1;
                kvargs.Debug (1, 1) logical = false;
            end

            self.NumScoutBees = kvargs.NumScoutBees;
            self.NumBestSolutions = kvargs.NumBestSolutions;
            self.NumBestSolutionNeighbors = kvargs.NumBestSolutionNeighbors;
            self.NumEliteSolutions = kvargs.NumEliteSolutions;
            self.NumEliteSolutionNeighbors = kvargs.NumEliteSolutionNeighbors;
            self.MaxIter = kvargs.MaxIter;
            self.Algorithms = kvargs.Algorithms;
            self.Debug = kvargs.Debug;
            self.ProbabilityToChooseWorseNeighbor = kvargs.ProbabilityToChooseWorseNeighbor;
        end
    end
